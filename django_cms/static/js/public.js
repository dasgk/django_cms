jQuery(function ($) {
    ...
    $('.ajaxForm').submit(function () {
        ...
        var loading = layer.msg('系统处理中...', {icon: 16, time: 0,shade: [0.3, '#393D49']});
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                if (textStatus == 'error') {
                    ...
                    for (var i in data) {
                        re=new RegExp(".","g");
                        var newstart=i.replace(re,"t");
                        var $ele = $('[name=' + newstart + ']');
                        ...
                        /*layer.msg(errormsg, {icon: 5, scrollbar: false, time: 2000, shade: [0.3, '#393D49']});*/
                        return false;
...
function html_close(url){
    var index=parent.layer.getFrameIndex(window.name);
    parent.location.href=url;
    parent.layer.close(index);
}
function myform_post(type,url){
    var loading = layer.msg('系统处理中...', {icon: 16, time: 0,shade: [0.3, '#393D49']});
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            //结果处理
            if(data.status){
                layer.msg('操作成功', {icon: 6, scrollbar: false, time: 1000, shade: [0.3, '#393D49']});
                setTimeout(function(){
                    if(type==1){
                        html_close(url);
                    }
